...
class Thread implements Runnable {
    ...
    private void init(ThreadGroup g, Runnable target, String name,
        ...
        if (g == null) {
            ...
            // 安全管理器不为null
            if (security != null) {
                // 获取SecurityManager中的ThreadGroup
        ...
        // 初始化当前线程对象的线程组
        this.group = g;
        // 初始化当前线程对象是否为守护线程，这里跟父线程保持一致
        this.daemon = parent.isDaemon();
        // 初始化当前线程的优先级，初始化时跟父线程一致
        ...
        /* Set thread ID */
        // 初始化当前线程的的线程ID, 该方法是同步的，内部实际上是threadSeqNumber++
        tid = nextThreadID();
    }
    ...
    public synchronized void start() {
        ...
        try {
            ...
            try {
                // 如果未成功启动
                if (!started) {
                    // 线程组内移除当前启动的线程数量-1, 同时启动失败的线程数量+1
     ...
     * 调用中断方法是由目标线程自己决定是否中断，而如果同时调用了wait,join,sleep等方法，
     * 会使当前线程进入阻塞状态，此时有可能发生InterruptedException异常
     *
    ...
    public final void setDaemon(boolean on) {
        // 检查安全权限
        checkAccess();
        // 本地方法，判断线程是否存活，如果一个线程已经启动并且尚未死亡，则该线程处于活动状态
        if (isAlive()) {
    ...
    public enum State {
         ...
         * </ul>
         * 正在等待另一个线程执行动作，直到指定等待时间的线程处于此状态
         */
        TIMED_WAITING,
         ...
         * 已退出的线程处于此状态，此时线程无法再回到 RUNNABLE 状态
         */
        TERMINATED;
    }
